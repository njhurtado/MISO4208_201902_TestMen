...
exports.default = (function (apiUrl, httpClient) {
    ...
    var convertHTTPResponse = function (response, type, resource, params) {
        ...
        switch (type) {
            ...
            case react_admin_1.CREATE:
                    json["id"]=json._id;
                return { data: __assign({}, params.data, { id: json._id }) };
            case react_admin_1.DELETE_MANY:
                return { data: json.map(function(x) {
                    //x => ({id:x._id, ...x})
                    x["id"]=x._id;
                    return x;
                }) || [] };
    ...
    return function (type, resource, params) {
        ...
        if (type === react_admin_1.UPDATE_MANY) {
            return Promise.all(params.ids.map(function (id) {
                ...
                data: responses.map(function (response) { return response.json.map(function(x) {
                    //x => ({id:x._id, ...x})
                    x["id"]=x._id;
                    return x;
                }); }),
        ...
        if (type === react_admin_1.DELETE_MANY) {
            return Promise.all(params.ids.map(function (id) {
                ...
                data: responses.map(function (response) { 
                    ...
                    return response.json.map(function(x) {
                        //x => ({id:x._id, ...x})
                        x["id"]=x._id;
                        return x;
                    }); }),
